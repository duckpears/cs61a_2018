...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min(centroids, key = lambda d: distance(d, location))
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    cluster_list = [[centroid_r(r), r] for r in restaurants]
...
def find_centroid(cluster):
    ...
    latitude, longitude = zip(*locations)
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    x_means = [x - mean(xs) for x in xs]
    y_means = [y - mean(ys) for y in ys]
    
    S_xx = sum([pow(i, 2) for i in x_means])
    S_yy = sum([pow(i, 2) for i in y_means])
    S_xy = sum([i * j for i, j in zip(x_means, y_means)])
