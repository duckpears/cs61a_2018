...
class Fib():
    ...
    def next(self):
        ...
        f.next_value = f.value + self.value
        return f
...
class VendingMachine:
    ...
    def deposit(self, amount):
        ...
        return 'Current balance: ${0}'.format(self.amount_deposited)
        
    def vend(self):
        if self.stock == 0:
            return 'Machine is out of stock.'
        elif self.amount_deposited < self.cost:
            return 'You must deposit ${0} more.'.format(self.cost - self.amount_deposited)
        elif self.amount_deposited > self.cost:
            balance = self.amount_deposited - self.cost
            self.amount_deposited = 0
            self.stock -= 1
            return 'Here is your {0} and ${1} change.'.format(self.item, balance)
        else:
            self.amount_deposited = 0
            self.stock -= 1
