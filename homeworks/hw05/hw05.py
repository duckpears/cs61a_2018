...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
      ...
      replacement = [replace_leaf(tree, old, new) for tree in branches(t)]
...
def is_weight(w):
    ...
    return not is_mobile(w) and type(label(w)) == int and len(branches(w)) == 0
...
def balanced(m):
    ...
    if is_weight(m):
      ...
      bal = [balanced(end(s)) for s in sides(m)]
      weights = [total_weight(end(s)) for s in sides(m)]
      lengths = [length(s) for s in sides(m)]
      
      return all(bal) and len(set(x * y for x, y in zip(weights, lengths))) == 1
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, p):
      if len(incorrect_password) >= 3:
        return "Your account is locked. Attempts: {}".format(str(incorrect_password))
      elif p != password:
        incorrect_password.append(p)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def new_withdraw(amount, p):
      if p not in [old_password, new_password]:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert 0 not in range(lower_bound(y), upper_bound(y) + 1), "Interval spans 0"
...
def check_par():
    ...
    True
    """
    r1 = interval(1, 1) # Replace this line!  0.25, 0.5
    r2 = interval(1, 1) # Replace this line!  0.25, 0.5
    return r1, r2

def multiple_references_explanation():
    return """The multiple reference problem exists. par2 is better than par1 because it does not combine the same interval twice, no variable that represents ans uncertain number is repeated, and the true value is fixed."""
...
def polynomial(x, c):
    ...
    def improve(update, close, guess, max_updates = 100):
      """
      Iteratively improve guess with update until close(guess) is true or max_updates have been applied.
      """
      k = 0
    ...
    return interval(round(min(values), 3), round(max(values), 3))
